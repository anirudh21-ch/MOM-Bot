...
def get_summary():
    """
    Generate a summary of the last processed transcript using OpenAI.
    
    Returns:
        JSON: Generated summary of the transcript.
    """
    global last_transcript
    
    if not last_transcript:
        return jsonify({"error": "No transcript available. Please upload and transcribe an audio file first."}), 400
    
    try:
        from src.openai_integration import summarize_transcript
        summary = summarize_transcript(last_transcript)
        return jsonify({"summary": summary, "transcript_segments": len(last_transcript)})
    except Exception as e:
    ...
    return jsonify(
        {
            "MOMbot API Documentation": {
                ...
                "endpoints": {
                    ...
                    "GET /summary": {
                        "description": "Generate AI summary of the last processed transcript",
                        "parameters": "None",
...
if __name__ == "__main__":
    ...
    print("  GET  /summary    - Generate AI summary of transcript")
